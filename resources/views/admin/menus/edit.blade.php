    ...
    <form method="POST" action="{{ route('admin.menus.update', $menu) }}">
        ...
        <div class="form-group">
            ...
            <div class="input-group{{ $errors->has('dish_id') ? ' is-invalid' : '' }}">
                <input type="text" name="dish_id" id="dish_id" list="dish-options" class="form-control{{ $errors->has('dish_id') ? ' is-invalid' : '' }}"
                       value="{{ old('dish_id', $menu->dish_id) }}" required>
                <div class="input-group-append">
                    <a href="{{ route('admin.dishes.create') }}" class="btn btn-outline-success">{{ __('Add dish') }}</a>
                </div>
                <datalist id="dish-options">
                    @foreach($dishes as $dish)<option value="{{ $dish->id }}">{{ $dish->category->name.' - '.$dish->name }}</option>@endforeach
                </datalist>
            </div>
        ...
        <div class="form-group">
            ...
            <input type="number" name="price" id="price" class="form-control{{ $errors->has('price') ? ' is-invalid' : '' }}"
                   value="{{ old('price', $menu->price) }}" min="1" step="1" required>
            @if($errors->has('price'))
        ...
        <div class="form-group">
            ...
            <input type="date" name="actual_at" id="actual_at" class="form-control{{ $errors->has('actual_at') ? ' is-invalid' : '' }}"
                   value="{{ old('actual_at', $menu->actual_at->format('Y-m-d')) }}" min="{{ now()->format('Y-m-d') }}" step="1" required>
        ...
        <div class="form-group">
            <button type="submit" class="btn btn-primary"{{ $menu->actual_at->format('Y-m-d') > now()->format('Y-m-d') ?: ' disabled' }}>{{ __('Save') }}</button>
        </div>
    </form>
@endsection