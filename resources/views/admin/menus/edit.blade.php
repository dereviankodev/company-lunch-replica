    ...
    <form method="POST" action="{{ route('admin.menus.update', $menu) }}">
        ...
        <div class="form-group">
            ...
            <input type="date" name="actual_at" id="actual_at"
                   ...
                   value="{{ old('actual_at', $menu->actual_at->format('Y-m-d')) }}" min="{{ now()->format('Y-m-d') }}"
                   step="1" required>
        ...
        <div class="form-group">
            <button type="submit"
                    class="btn btn-primary"{{ $menu->actual_at->format('Y-m-d') > now()->format('Y-m-d') ?: ' disabled' }}>{{ __('Save') }}</button>
        </div>
    </form>
@endsection